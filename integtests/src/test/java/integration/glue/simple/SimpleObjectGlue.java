...
public class SimpleObjectGlue extends CukeGlueAbstract {
    ...
    public void I_create_a_new_simple_object() throws Throwable {
        if(supportsMocks()) {
            checking(new Expectations() {
                {
                    oneOf(service(SimpleObjects.class)).create(with(any(String.class)), with(any(Integer.class)));
                    will(addToInMemoryDB());
                }
            });
        }
        service(SimpleObjects.class).create(UUID.randomUUID().toString(), UUID.randomUUID().variant());
    }
